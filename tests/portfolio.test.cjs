...
test('language switch translates case studies without closing them', () => withPage(async page => {
  ...
  assert.equal(await page.locator('.project-case summary').first().getAttribute('aria-label'), 'How I built it: Multi-Market');
  await page.locator('[data-language="es"]').click();
  assert.equal(await text(page, '.project-case dt >> nth=0'), 'El problema');
  assert.equal(await page.locator('.project-case').first().getAttribute('open'), '');
}));

test('persisted English initializes the portfolio with complete translations', () => withPage(async page => {
...
for (const width of [1440, 980, 760, 390, 320]) {
  test(`responsive layout at ${width}px keeps page, filters and expanded content in bounds`, () => withPage(async page => {
    ...
    for (const selectControl of await page.locator('.market-filters select').all()) {
...
test('market shows six illustrated sample products with ARS prices and resets its shop filter', () => withPage(async page => {
...
test('a missing market script leaves a fallback and native case studies still work', () => withPage(async page => {
  assert.equal(await page.locator('.market-fallback').isVisible(), true);
  assert.equal(await page.locator('#market-search').isEnabled(), false);
  assert.equal(await page.locator('.market-results').isVisible(), false);
  await page.locator('.project-case summary').first().click();
